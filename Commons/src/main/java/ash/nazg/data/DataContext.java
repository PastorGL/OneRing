...
public class DataContext {
    ...
    public DataContext(final JavaSparkContext sparkContext) {
        ...
        store.put(Constants.METRICS_DS, new DataStream(StreamType.Columnar, sparkContext.emptyRDD(), Collections.singletonMap("value", METRICS_COLUMNS)));
    }
    ...
    public void createDataStream(String inputName, Map<String, Object> params) {
        ...
        inputRdd.rdd().setName("one-ring:input:" + inputName);
    ...
    private void save(String outputName, Map<String, Object> params, DataStream ds) {
        ...
        rdd.rdd().setName("one-ring:output:" + outputName);
    ...
    public JavaRDDLike select(boolean distinct, List<String> inputs, UnionSpec unionSpec, JoinSpec joinSpec, final boolean star, List<SelectItem> items, QueryItem query, Double limitPercent, Long limitRecords, VariablesContext variables) {
        ...
        if (unionSpec != null) {
            ...
            for (int i = 1; i < inpNumber; i++) {
                ...
                if (!streamI.accessor.attributes("value").containsAll(resultAccessor.attributes("value"))
                        || !resultAccessor.attributes("value").containsAll(streamI.accessor.attributes("value"))) {
            ...
            resultAccessor = StreamType.KeyValue.accessor(Collections.singletonMap("value", stream0.accessor.attributes("value").stream()
            ...
            for (int r = 1; r < inputs.size(); r++) {
                ...
                if (joinSpec == JoinSpec.RIGHT_ANTI) {
                    resultAccessor = StreamType.KeyValue.accessor(Collections.singletonMap("value", store.get(inputR).accessor.attributes("value").stream()
                    ...
                    Stream<String> concat = Stream.concat(
                            resultAccessor.attributes("value").stream(),
                            store.get(inputR).accessor.attributes("value").stream()
                                    .map(e -> inputR + "." + e)
                    );
                    resultAccessor = StreamType.KeyValue.accessor(Collections.singletonMap("value", concat
        ...
        if (sourceRdd instanceof JavaRDD) {
            switch (resultType) {
                ...
                case Track: {
                    ...
                    if ("track".equals(query.category)) {
                        queryTrack = true;
                    }
                    if ("segment".equals(query.category)) {
                        querySegment = true;
                    }
                    if ("point".equals(query.category)) {
                        queryPoint = true;
                    }
                                ...
                                while (it.hasNext()) {
                                        ...
                                        if (Operator.bool(trackPropGetter, _query.expression, vc)) {
                                            if (star) {
                                                ...
                                                for (int i = 0; i < size; i++) {
                                                    ...
                                                    if ("track".equals(selectItem.category)) {
                                    ...
                                    for (int j = segments.length - 1; j >= 0; j--) {
                                        ...
                                        for (int i = 0; i < size; i++) {
                                            ...
                                            if ("segment".equals(selectItem.category)) {
                                    ...
                                    for (int k = segments.length - 1; k >= 0; k--) {
                                        ...
                                        for (int j = points.length - 1; j >= 0; j--) {
                                            ...
                                            for (int i = 0; i < size; i++) {
                                                ...
                                                if ("point".equals(selectItem.category)) {
