...
public class TDL4Interpreter implements Iterable<TDL4.StatementContext> {
    ...
    private void select(TDL4.Select_stmtContext ctx) {
        ...
        if (star) {
            if (join != null) {
                for (String fromName : fromSet) {
                    columns.addAll(dataContext.get(fromName).accessor.attributes("value"));
                }
            } else {
                columns.addAll(firstStream.accessor.attributes("value"));
            }
        } else {
        ...
        if (star && (union == null) && (join == null) && (query.expression == null)) {
            ...
            dataContext.put(intoName, new DataStream(firstStream.streamType, result, Collections.singletonMap("value", columns)));
        }
    }
    ...
    private void analyze(TDL4.Analyze_stmtContext ctx) {
        ...
        for (Map.Entry<String, DataStream> e : dataContext.getAll(dsName).entrySet()) {
            ...
            List<String> columns = ds.accessor.attributes("value");
    ...
    public Map<String, Object> resolveParams(TDL4.Params_exprContext params) {
        ...
        if (params != null) {
            for (TDL4.ParamContext atRule : params.param()) {
                ...
                ret.put(parseIdentifier(atRule.property_name().getText()), obj);
            }
        }
